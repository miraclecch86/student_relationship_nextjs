...
export async function middleware(request: NextRequest) {
  ...
  if (user) {
    console.log('[Middleware FINAL] User exists. User ID:', user.id, 'Metadata Role:', user.user_metadata?.role);
  }

  // 로그인, 인증 콜백, 선생님 이름 입력 경로는 일단 허용
  if (pathname.startsWith('/login') || pathname.startsWith('/auth') || pathname === '/teacher-setup') {
    ...
    if (user && user.user_metadata?.role && user.user_metadata?.teacher_name) {
      ...
      if (pathname.startsWith('/login') || pathname === '/teacher-setup') {
        ...
        return NextResponse.redirect(new URL(userRole === 'teacher' ? '/teacher' : '/student', request.url));
      }
    }
  ...
  if (!user) {
    console.error('[Middleware FINAL] No session, redirecting to login from protected path:', pathname);
  ...
  if (!userRole) {
    ...
    try {
      ...
      if (profileError && profileError.code !== 'PGRST116') {
        ...
        return NextResponse.redirect(new URL('/login?error=profile_fetch_failed_mw', request.url));
      }
      ...
      return NextResponse.redirect(new URL('/login?error=profile_fetch_exception_mw', request.url));
    }
  }
  ...
  if (pathname.startsWith('/teacher') && userRole !== 'teacher') {
    ...
    return NextResponse.redirect(new URL('/login?error=unauthorized_teacher', request.url));
  }
