                    ...
                    <div className="flex flex-col items-center justify-center h-full overflow-y-auto">
                      ...
                      >
                        ...
                        <span className="mt-2 block text-base font-semibold text-gray-900">
                          {files.length > 0 ? "파일 추가 첨부" : "문서/사진 추가"}
                        </span>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 h-full min-h-0">
                      ...
                      <div className="w-full md:w-1/2 flex flex-col bg-gray-50 rounded-xl border border-gray-200 overflow-hidden min-h-0 h-full">
                          <div className="p-3 bg-gray-100 border-b border-gray-200 font-medium text-sm text-gray-700 flex justify-between items-center shadow-sm z-10 flex-shrink-0">
                              <span>원본 문서 ({files.length}건)</span>
                              <span className="text-xs font-normal text-gray-500">참고/대조용</span>
                          </div>
                      ...
                      <div className="w-full md:w-1/2 flex flex-col h-full bg-white min-h-0">
                        <div className="mb-4 flex-shrink-0">
                            <h4 className="text-lg font-bold text-gray-900 border-b border-gray-200 pb-2">✨ 추출된 주관식 질문</h4>
                            <p className="text-xs text-gray-500 mt-2">AI가 내용을 인식했습니다. 원하는 대로 내용을 편집하거나 순서를 조정해 주세요.</p>
                        </div>

                        <div className="flex-grow overflow-y-auto pr-3 pb-4 custom-scrollbar min-h-0">
                            <div className="space-y-3">
                                    {extractedQuestions.map((q, idx) => (
                                <div key={idx} className="flex gap-2 items-start bg-white p-3 rounded-xl border border-gray-200 shadow-sm group hover:border-indigo-300 transition-colors">
                                    ...
                                    <div className="flex-grow">
                                        <textarea
                                            ...
                                            rows={2}
                                            className="w-full p-2 border border-blue-100 bg-blue-50/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white text-sm resize-none transition-all"
                                            placeholder="질문 내용을 입력하세요"
                                        />
                                    </div>
                                    <button onClick={() => removeQuestion(idx)} className="mt-1 p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors" title="삭제"><TrashIcon className="w-5 h-5"/></button>
                                </div>
                            ))}
                            
                                    <button 
                                        onClick={handleAddEmptyQuestion}
                                        className="w-full mt-2 py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-600 transition-colors flex justify-center items-center gap-2 text-sm font-medium"
                                    >
                                        <PlusIcon className="w-4 h-4" /> 빈 질문 추가하기
                                    </button>
                            </div>
                        </div>
                      </div>
